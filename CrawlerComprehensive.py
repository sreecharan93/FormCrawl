...
for i in range(len(cikList)):
	base_url = "http://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK="+str(cikList[i])+"&owner=include&count=100&hidefilings=0"	
	r = requests.get(base_url)
	data = r.text
	...
	linkListFinal = [] # List of links containing only the required documents
	# Retrieve only elements that end with index.html
	for x in range(len(linkList)):
		elements = linkList[x].split("-")
		if "index.html" in elements:
			linkListFinal.append(linkList[x])
